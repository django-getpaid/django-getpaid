...
def test_get_flow_begin(
    payment_factory, settings, live_server, requests_mock, rf
):
...
def test_rest_flow_begin(
    ...
    payment = payment_factory(external_id=uuid.uuid4())
    requests_mock.post(
        str(url_api_register), json={'url': str(url_post_payment)}
    )
...
def test_pull_flow_paid(
    ...
    url_get_status = reverse(
        'paywall:get_status', kwargs={'pk': payment.external_id}
    )
