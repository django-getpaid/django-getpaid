...
def test_verified_submerchant_status(
    getpaid_client, requests_mock
):
    ext_id = "test123"
    url = urljoin(
...
def test_not_verified_submerchant_status(
    getpaid_client, requests_mock
):
    ext_id = "test123"
    url = urljoin(
...
def test_submerchant_operations(
    getpaid_client, requests_mock
):
    ...
    url = urljoin(
        getpaid_client.api_url,
        f"/api/v2_1/customers/ext/{ext_id}/operations"
    )
    url += "?" + urlencode({
        "currencyCode": "PLN",
