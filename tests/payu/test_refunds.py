...
def start_refund_response_mock(requests_mock):
    mock = {
        ...
        "refund": {
            "dispositions": {
                "instrumentAmount": "1000", "walletAmount": "0", "couponAmount": "0"
            },
            "refundId": "5000006708",
            "extRefundId": "0685691c-6c42-4448-b100-3e9e1b2e7a56",
            "amount": "1000",
            "currencyCode": "PLN", "description": "Uznanie 5000006708 Zwrot",
            "creationDateTime": "2020-05-28T21:21:28.517+02:00", "status": "PENDING",
            "statusDateTime": "2020-05-28T21:21:28.683+02:00"
        },
        "status": {"statusCode": "SUCCESS", "statusDesc": "Refund queued for processing"}
    }
    requests_mock.post(
        f"/api/v2_1/orders/{ORDER_ID}/refunds",
        json=mock,
        status_code=200,
    )
...
def test_refund(getpaid_client, start_refund_response_mock):
    result = getpaid_client.refund(
        order_id=ORDER_ID,
        ext_refund_id="random_payment_id"
    )
...
def test_refund_failure(response_status, getpaid_client, requests_mock):
    ...
    with pytest.raises(RefundFailure):
        getpaid_client.refund(
            order_id=ext_order_id,
            ext_refund_id="random_id"
        )
